...
public class Member {
    ...
    public Boolean setP_number(long p_number) {
        // Checking value and length of number for simple validation
        if(this.pers_number == 0 && p_number == -1 || Long.toString(p_number).length() < p_numberLength ||
                Long.toString(p_number).length() > p_numberLength){
            // Keeps addMember from inputting empty field.
            return false;
        }else {
